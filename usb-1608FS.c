...
int usbAInScan_USB1608FS(HIDInterface* hid[], __u8 lowchannel, __u8 highchannel, __u32 nSamples,
  ...
  *frequency = 1.0e7/(preload*(1<<arg.prescale));
  ...
  for (i = 0; i < nScans; i++, pipe = (pipe)%6 + 1) {  //pipe should take the values 1-6
    ...
    if (i != data.scan_index) {
      printf("AInScan_USB1608FS: ret = %d  scan = %d   nSamples = %d   scan_index = %d  Endpoint = %#x  \n",
    	   ret, i, nSamples, data.scan_index, USB_ENDPOINT_IN |(pipe+2)); 
    }
