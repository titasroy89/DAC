...
int usbAInScanRead_USB20X(usb_dev_handle *udev, int nScan, int nChan, __u16 *data)
{
  ...
  if (ret != nbytes) {
    perror("usbAInScanRead_USB20X: error in usb_bulk_read.");
  }
...
void usbAInBulkFlush_USB20X(usb_dev_handle *udev, __u8 count)
{
  /* This comman will send a specified number of empty bulk IN packets. */
  __u8 requesttype = (DEVICE_TO_HOST | VENDOR_TYPE | DEVICE_RECIPIENT);
  usb_control_msg(udev, requesttype, AIN_BULK_FLUSH, 0x0, 0x0, (char *) &count, 0x1, HS_DELAY);
}

/***********************************************
 *            Analog Output                    *
 ***********************************************/
void usbAOut_USB20X(usb_dev_handle *udev, __u8 channel, __u16 value)
{
  ...
  usb_control_msg(udev, requesttype, AOUT, (int) value, (int) channel, (char *) 0x0, 0x0, HS_DELAY);
  return;
}
