...
int main (int argc, char **argv)
{
  ...
  while(1) {
	...
	usbDTristateW_USB1208FS_Plus(udev, PORTA, 0x1);
	usbDTristateW_USB1208FS_Plus(udev, PORTB, 0x0);
	printf("Digital Port Tristate Register = %#x\n", usbDTristateR_USB1208FS_Plus(udev, PORTA));
	do {
          printf("Enter a  number [0-0xff] : " );
          scanf("%x", &temp);
          usbDLatchW_USB1208FS_Plus(udev, PORTB, (__u8)temp);
          input = usbDPort_USB1208FS_Plus(udev, PORTA);
	  temp = usbDLatchR_USB1208FS_Plus(udev, PORTB);
          printf("The value read on Port A = %#x   Latched value on PORT B = %#x\n\n",input, temp);
        } while (toContinue());
        break;
      case 'i':
